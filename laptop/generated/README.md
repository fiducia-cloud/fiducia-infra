...
Checked-in artifacts produced by `tools/render-laptop-fleet.mjs`.
Regenerate with `node tools/render-laptop-fleet.mjs`; CI's `--check` fails on drift.

- `edge-regions.json` — the `FIDUCIA_REGIONS` list for the laptop simulation
  clusters (`laptop-aws-sim`, `laptop-gcp-sim`, `laptop-azure-sim`).

A directory called `generated` is not automatically a freeze tree: tool caches,
Flutter/Xcode `.generated/`, and Gradle `build/` output stay writable and are
declared `writable` or `ignored` instead. This tree is different — its artifact is
machine-written *and* tracked, so it is frozen like the rest.
