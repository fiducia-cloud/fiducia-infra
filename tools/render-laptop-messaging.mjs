...
export function renderNatsConfigMapPatch(natsConfig) {
  const indented = natsConfig
    .replace(/\n$/, "")
    .split("\n")
    .map((line) => `    ${line}`)
    .join("\n");
  return `# GENERATED by tools/render-laptop-messaging.mjs from laptop/topology.toml — do not edit.
# Strategic-merge patch for the hand-authored base ConfigMap. A Kustomize
# generator cannot use behavior=replace against a resource loaded from YAML.
apiVersion: v1
kind: ConfigMap
metadata:
...
export function renderLaptopMessaging() {
  ...
  const files = {};
  for (const cluster of topology.cluster) {
    const natsConfig = renderNatsConfig(cluster, topology);
    const clusterRoot = `laptop/clusters/${cluster.name}`;
    files[`${clusterRoot}/nats.conf`] = natsConfig;
    files[`${clusterRoot}/nats-config.patch.yaml`] = renderNatsConfigMapPatch(natsConfig);
  }
