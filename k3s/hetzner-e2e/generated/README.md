...
Checked-in artifacts produced by `tools/render-hetzner-e2e.mjs`.
Regenerate with `node tools/render-hetzner-e2e.mjs`; CI's `--check` fails on drift.

- `lb-endpoints.json` — the public load-balancer endpoints for the Hetzner k3s
  end-to-end fleet, consumed by the Cloudflare edge.
